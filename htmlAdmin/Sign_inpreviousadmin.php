...
global $db; 
    ...
    if(!empty($spass) && !empty($cpass) && !empty($semail))
    {          
        if($spass == $cpass)
        {  
            $options = ['cost' => 12,];
            ...
            $c = $db->prepare("SELECT email from adminn  WHERE email = :email");
            ...
            //echo "<br>Result1 == $result1";
                ...
                if($result1 == 0)
                {
                        ...
                        $q = $db->prepare("INSERT INTO adminn ( email, pass) VALUES ( :email, :pass)");
                        $q->execute([
                        'email' => $semail,
                        'pass' => $hash_pass,
                        ]); 
